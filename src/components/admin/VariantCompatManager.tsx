...
export function VariantCompatManager({ variantId, variantSize }: { variantId: string; variantSize: string }) {
  ...
  const models = (vehQ.data?.models ?? []).filter((m: any) =>
    (!makeId || m.make_id === makeId) && (!q || m.name.toLowerCase().includes(q.toLowerCase()))
  );
  const years = (vehQ.data?.years ?? []).filter((y: any) => selected.length === 1 ? y.model_id === selected[0] : selected.includes(y.model_id));
  const modelName = (id: string) => (vehQ.data?.models ?? []).find((m: any) => m.id === id)?.name ?? id;
  ...
  const mAdd = useMutation({
    mutationFn: () => add({ data: {
      variant_id: variantId, make_id: makeId, vehicle_model_ids: selected,
      year_id: yearId || null,
    } }),
    onSuccess: (r) => { qc.invalidateQueries({ queryKey: ["adm-var-compat", variantId] }); setSelected([]); setYearId(""); setMsg(`Added ${r.added} vehicle${r.added === 1 ? "" : "s"}.`); },
    ...
    <div className="mt-8 border-t border-border pt-6">
      ...
      <div className="mt-4 card-surface bg-white p-4">
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <label className="text-sm"><span className="mb-1 block font-medium">Search models</span>
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="e.g. Corolla"
              className="h-10 w-full rounded-md border border-border px-3 text-sm" disabled={!makeId} />
          </label>
        </div>

        {makeId && (
          <div className="mt-3">
            <div className="mb-1 text-xs font-medium text-muted-foreground">Models (select one or more)</div>
            <div className="flex max-h-40 flex-wrap gap-1.5 overflow-y-auto rounded-md border border-border bg-surface-2 p-2">
              {models.map((m: any) => {
                const on = selected.includes(m.id);
                return <button key={m.id} type="button" onClick={() => { setSelected(on ? selected.filter(x => x !== m.id) : [...selected, m.id]); setYearId(""); }}
                  className={`rounded-full border px-3 py-1 text-xs ${on ? "border-primary bg-primary/10 text-primary" : "border-border bg-white"}`}>{m.name}</button>;
              })}
